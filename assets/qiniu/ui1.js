...
function FileProgress(file, targetID) {
    ...
    if (!this.fileProgressWrapper.length) {
        ...
        //var le =$('.progressFileSize').length;
        //zSize =0;
        //for(i=0;i<le;i++){
        //    zSizs =zSize + fileSize(i);
        //    alert(fileseze(i));
        //}
...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    if (res.url) {
        ...
        str = "<div name='imgUrlDiv'><strong>文件鏈接:</strong><a href=" + url + " target='_blank' > " + link + "</a></div>" +
            "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>"+
            "<div><strong>已上傳的文件名稱:</strong>&nbsp;&nbsp;&nbsp;<b style='font-weight: bold; color: #333;'>"+ encodeURI(res.key)+"</b></div>";
    }
    ...
    if (!isImg) {
        ...
        url = domain + encodeURI(res.key);
        var link = domain + res.key;
        showImg.attr('src', url+'?vframe/jpg/offset/2');//視頻截圖

        linkWrapper.attr('href', url+'?vframe/jpg/offset/2').attr('title', '查看原图');
